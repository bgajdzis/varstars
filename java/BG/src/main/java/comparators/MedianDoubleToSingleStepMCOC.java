...
public class MedianDoubleToSingleStepMCOC extends AbstractMonolithicCOComparator<VarstarFeatureSet> {


    public MedianDoubleToSingleStepMCOC(String name) throws ComparatorException {
        super(name);
    }

    @Override
    public Integer getUniqueId() {
        return Constants.MedianDoubleToSingleStep;
    }
    ...
    protected double getMembershipValue(IMonolithicReferenceObject ref) throws ComparatorException {
        ...
        try {
            inputValue = ((VarstarsCO) getInputObject()).getMedianDoubleToSingleStep();
            referenceValue = ((VarstarsRef) ref).getMedianDoubleToSingleStep();
