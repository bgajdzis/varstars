...
public class TotalTimeMCOC extends AbstractMonolithicCOComparator<VarstarFeatureSet> {


    public TotalTimeMCOC(String name) throws ComparatorException {
        super(name);
    }

    @Override
    public Integer getUniqueId() {
        return Constants.TotalTime;
    }
    ...
    protected double getMembershipValue(IMonolithicReferenceObject ref) throws
        ...
        try {
            inputValue = ((VarstarsCO) getInputObject()).getTotalTime();
            referenceValue = ((VarstarsRef) ref).getTotalTime();
