...
public class FreqNAliasMCOC extends AbstractMonolithicCOComparator<VarstarFeatureSet> {


    public FreqNAliasMCOC(String name) throws ComparatorException {
        super(name);
    }

    @Override
    public Integer getUniqueId() {
        return Constants.FreqNAlias;
    }
    ...
    protected double getMembershipValue(IMonolithicReferenceObject ref) throws
        ...
        try {
            inputValue = ((VarstarsCO) getInputObject()).getFreqNAlias();
            referenceValue = ((VarstarsRef) ref).getFreqNAlias();
